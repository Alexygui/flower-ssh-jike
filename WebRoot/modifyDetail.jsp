    ...
    <div class="content">
		...
		<div class="right">
			...
			<s:form action="updateUserDetail.action" method="post">
				<!-- %用于计算大括号内的ognl表达式的值，此处调用session中保存的user对象 -->
				...
				<tr><td></td><td>
				<ss:datetimepicker label="出生日期" value="%{#session.user.userdetail.csrq}" name="userdetail.csrq"></ss:datetimepicker>
				</td></tr>
				<s:textfield label="E-mail" value="%{#session.user.userdetail.email}" name="userdetail.email"></s:textfield>
				<s:textfield label="手机" value="%{#session.user.userdetail.phone}" name="userdetail.phone"></s:textfield>
				<s:textfield label="地址" value="%{#session.user.userdetail.address}" name="userdetail.address"></s:textfield>
